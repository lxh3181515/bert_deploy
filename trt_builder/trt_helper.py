...
class TrtNetworkHelper():
    ...
    def addLayerNorm(self, x, gamma, beta, layer_name=None, precision=None, mark_output=False):
        # TODO: create your layer norm plugin
        SCALE = self.network.add_constant((1, 1, gamma.shape[-1]), trt.Weights(gamma))
        BIAS = self.network.add_constant((1, 1, beta.shape[-1]), trt.Weights(beta))
        
        X_norm = self.network.add_plugin_v2([x], self.get_plugin('LayerNorm'))
        if mark_output:
            self.markOutput(X_norm.get_output(0))
        X_mul = self.network.add_elementwise(X_norm.get_output(0), SCALE.get_output(0), trt.ElementWiseOperation.PROD)
        trt_layer = self.network.add_elementwise(X_mul.get_output(0), BIAS.get_output(0), trt.ElementWiseOperation.SUM)
    ...
    def addLinear(self, x, weight, bias, layer_name=None, precision=None):
        ...
        WEIGHT = self.network.add_constant((1, weight.shape[0], weight.shape[1]), trt.Weights(np.ascontiguousarray([weight], dtype=np.float32)))
        BIAS = self.network.add_constant((1, 1, bias.shape[-1]), trt.Weights(np.ascontiguousarray([bias], dtype=np.float32)))
        X_mat_mul = self.network.add_matrix_multiply(x, trt.MatrixOperation.NONE, 
                                                     WEIGHT.get_output(0), trt.MatrixOperation.NONE)
        trt_layer = self.network.add_elementwise(X_mat_mul.get_output(0), BIAS.get_output(0), trt.ElementWiseOperation.SUM)
    ...
    def addSoftmax(self, x: trt.ITensor, dim: int = -1, layer_name=None, precision=None) -> trt.ITensor:
        ...
        if dim < 0:
            trt_layer.axes = 1 << (len(x.shape) - 1)
        else:
    ...
    def addScale(
        ...
        """scale"""
        # TODO: add scale
        trt_layer = self.network.add_scale(x, trt.ScaleMode.UNIFORM, scale=trt.Weights(np.ascontiguousarray([scale], dtype=np.float32)))
        
        if layer_name is None:
            layer_name = "nn.Scale"
        else:
            layer_name = "nn.Scale." + layer_name
    ...
    def addMatMul(self, a: trt.ITensor, b: trt.ITensor, layer_name: Optional[str] = None) -> trt.ITensor:
        ...
        if layer_name is None:
            layer_name = "nn.MatMul"
        else:
            layer_name = "nn.MatMul." + layer_name
...
class InferHelper():
    ...
    def infer(self, inputs: list):
        ...
        for i in range(nInput):
            ...
            # print('inputs[i].nbytes:', inputs[i].nbytes)
        ...
        for i in range(nOutput):
            ...
            # print('outputs[i].nbytes:', outputs[i].nbytes)
