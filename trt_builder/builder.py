...
def test_case_data(infer_helper, case_data_path):
    ...
    print(trt_outputs[0].dtype)
    print(trt_outputs[0])   

def main():
    parser = argparse.ArgumentParser(description="TensorRT BERT Sample", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument("-x", "--onnx", required=False, default="bert-base-uncased/model.onnx", help="The ONNX model file path.")
    parser.add_argument("-o", "--output", required=True, default="bert-base-uncased/model.plan", help="The bert engine file, ex bert.engine")
    parser.add_argument("-c", "--config-dir", required=True, default="bert-base-uncased",
